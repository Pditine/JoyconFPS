        ...
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                DebugPrint(string.Format(
                        "Dequeue. Queue length: {0}. Packet ID: {1:X2}. Timestamp: {2:X2}. Lag to dequeue: {3}. Lag between packets (expect 15ms): {4}",
                        reports.Count, 
                        report_buf[0], 
                        report_buf[1], 
                        System.DateTime.Now.Subtract(rep.GetTime()).TotalSeconds, 
                        rep.GetTime().Subtract(ts_prev).TotalSeconds), 
                    DebugType.THREADING);
                ts_prev = rep.GetTime();
            }
